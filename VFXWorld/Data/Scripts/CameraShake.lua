...
local startPosition = nil --camera:GetPositionOffset()
local endPosition = script:GetCustomProperty("EndPosition")
...
local setStartPosition = script:GetCustomProperty("SetStartPosition")
...
function Tick(deltaTime)
	...
	if progress == 0 then 
		camera = Game.GetLocalPlayer():GetDefaultCamera()
		startPosition = camera:GetPositionOffset() 
		
		--endPosition = Vector3.New(startPosition.x, startPosition.y, endPosition.z)
	end
	if progress == 0 and setStartPosition == true then  
		--script.parent:SetPosition(startPosition)
	end
	...
	if progress >= duration+startWaitTime then progress = duration+startWaitTime end 
	...
	local p = CoreMath.Clamp(progress-startWaitTime, 0, 1000000)
	...
	if endPosition.x == 0 then endPosition.x = camera:GetPositionOffset().x end 
	if endPosition.y == 0 then endPosition.y = camera:GetPositionOffset().y end 
	if endPosition.z == 0 then endPosition.z = camera:GetPositionOffset().z end 

	camera:SetPositionOffset(Vector3.Lerp(startPosition, endPosition, t)) 
...
function Reset()
	progress = 0
	isActive = false  
	camera:SetPositionOffset(startPosition)
end
